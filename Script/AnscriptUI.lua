...
if getgenv then
	local ok, result = pcall(function() return getgenv().DISABLE_Anscript_REQUESTS end)
	if ok and result then requestsDisabled = true end
	local ok2, result2 = pcall(function() return getgenv().Anscript_ASSET_ID end)
	if ok2 and type(result2) == "number" then customAssetId = result2 end
	local ok3, result3 = pcall(function() return getgenv().Anscript_SECURE end)
...
local function secureNotify(wType, title, content)
	...
	task.spawn(function()
		while not AnscriptLibrary or not AnscriptLibrary.Notify do task.wait(0.5) end
		AnscriptLibrary:Notify({
			Title = title,
			Content = content,
			Duration = 8,
		})
...
local AnscriptFolder = "Anscript"
local ConfigurationFolder = AnscriptFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		AnscriptOpen = {Type = 'bind', Value = 'K', Name = 'Anscript Keybind'},
		-- buildwarnings
		-- Anscriptprompts
...
local overriddenSettings: { [string]: any } = {} -- For example, overriddenSettings["System.AnscriptOpen"] = "J"
...
local function callSafely(func, ...)
	if func then
		...
		if not success then
			warn("Anscript | Function failed with error: ", result)
			return false
		else
			return result
		end
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		if callSafely(isfolder, AnscriptFolder) then
			if callSafely(isfile, AnscriptFolder..'/settings'..ConfigurationExtension) then
				file = callSafely(readfile, AnscriptFolder..'/settings'..ConfigurationExtension)
			end
		end
		...
		if useStudio then
			file = [[
	{"General":{"AnscriptOpen":{"Value":"K","Type":"bind","Name":"Anscript Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Anscript Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
]]
		end
			...
			for categoryName, categoryTable in file do
				for settingName, setting in categoryTable do
					...
					if not (settingType == typeof(setting.Value)) then
						warn("Anscript | Error parsing settings file. '"..settingName.."' must be a "..settingType)
						continue
					end
	...
	if not success then 
		if writefile then
			warn('Anscript had an issue accessing configuration saving capability.')
		end
	end
end
...
if not requestsDisabled and not useStudio then
	local fetchSuccess, fetchResult = pcall((game :: any).HttpGet, game, "https://raw.githubusercontent.com/SiriusSoftwareLtd/Anscript/refs/heads/main/reporter.lua")
		...
		if execSuccess and Analytics then
			pcall(function()
				reporter = Analytics.new({
					url          = "https://Anscript-collect.sirius-software-ltd.workers.dev",
					token        = ANALYTICS_TOKEN,
					product_name = "Anscript",
					category     = "UILibrary",
				})
			end)
		end
...
local AnscriptAssetId = customAssetId or 10804731440
local Anscript = useStudio and script.Parent:FindFirstChild('Anscript') or game:GetObjects("rbxassetid://"..AnscriptAssetId)[1]
local buildAttempts = 0
local correctBuild = false
local warned
local globalLoaded
local AnscriptDestroyed = false -- True when AnscriptLibrary:Destroy() is called

repeat
	if Anscript:FindFirstChild('Build') and Anscript.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Anscript | Build Mismatch')
		print('Anscript may encounter issues as you are running an incompatible interface version ('.. ((Anscript:FindFirstChild('Build') and Anscript.Build.Value) or 'No Build') ..').\n\nThis version of Anscript is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	local toDestroy
	toDestroy, Anscript = Anscript, useStudio and script.Parent:FindFirstChild('Anscript') or game:GetObjects("rbxassetid://"..AnscriptAssetId)[1]
...
if secureMode and not customAssetId then
	secureNotify("default_asset", "Secure Mode", "You are using the default Anscript asset ID. Set Anscript_ASSET_ID to a custom upload to avoid detection.")
end

do
	local AssetPath = AnscriptFolder.."/Assets"
	local AssetBaseURL = "https://github.com/SiriusSoftwareLtd/Anscript/blob/main/assets/"
		...
		local ok, err = pcall(function()
			...
			for id, _ in assetFiles do
				...
				if success then
					...
					warn("Anscript | Failed to load custom asset: "..tostring(id).." - "..tostring(asset))
				end
			end
		end)

		if not ok then
			warn("Anscript | Failed to load custom assets: "..tostring(err))
			secureNotify("asset_load_fail", "Anscript", "Failed to load custom assets. UI images may not display correctly.")
		end
	else
		secureNotify("no_getcustomasset", "Anscript", "Your executor does not support getcustomasset. Some UI images may not render correctly.")
	end


	Anscript.Main.Shadow.Image.Image = customAssets[tostring(5587865193)]
	Anscript.Main.Topbar.Hide.Image = customAssets[tostring(10137832201)]
	Anscript.Main.Topbar.ChangeSize.Image = customAssets[tostring(10137941941)]
	Anscript.Main.Topbar.Settings.Image = customAssets[tostring(80503127983237)]
	Anscript.Main.Topbar.Icon.Image = customAssets[tostring(78137979054938)]
	Anscript.Main.Topbar.Search.Image = customAssets["IconMagnifyingGlass2"]
	Anscript.Main.Topbar.Search.ImageRectOffset = Vector2.new(0, 0)
	Anscript.Main.Topbar.Search.ImageRectSize = Vector2.new(0, 0)
	Anscript.Main.Elements.Template.Toggle.Switch.Shadow.Image = customAssets[tostring(3602733521)]
	Anscript.Main.Elements.Template.Slider.Main.Shadow.Image = customAssets[tostring(3602733521)]
	Anscript.Main.Elements.Template.Dropdown.Toggle.Image = customAssets["IconChevronTopMedium"]
	Anscript.Main.Elements.Template.Dropdown.Toggle.ImageRectOffset = Vector2.new(0, 0)
	Anscript.Main.Elements.Template.Dropdown.Toggle.ImageRectSize = Vector2.new(0, 0)
	Anscript.Main.Elements.Template.Label.Icon.Image = customAssets[tostring(11745872910)]
	Anscript.Main.Elements.Template.ColorPicker.CPBackground.MainCP.Image = customAssets[tostring(11413591840)]
	Anscript.Main.Elements.Template.ColorPicker.CPBackground.MainCP.MainPoint.Image = customAssets[tostring(3259050989)]
	Anscript.Main.Elements.Template.ColorPicker.ColorSlider.SliderPoint.Image = customAssets[tostring(3259050989)]
	Anscript.Main.TabList.Template.Image.Image = customAssets[tostring(4483362458)]
	Anscript.Main.Search.Search.Image = customAssets[tostring(18458939117)]
	Anscript.Main.Search.Shadow.Image = customAssets[tostring(5587865193)]
	Anscript.Notifications.Template.Icon.Image = customAssets[tostring(77891951053543)]
	Anscript.Notifications.Template.Shadow.Image = customAssets[tostring(3523728077)]
	Anscript.Loading.Banner.Image = customAssets[tostring(111263549366178)]
...
if Anscript.AbsoluteSize.X < minSize.X and Anscript.AbsoluteSize.Y < minSize.Y then
...
local MPrompt = Anscript:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Anscript:FindFirstChild('Drag')
...
local Icons = useStudio and require(script.Parent.icons) or loadWithTimeout('https://raw.githubusercontent.com/SiriusSoftwareLtd/Anscript/refs/heads/main/icons.lua')
-- Variables
...
local keybindConnections = {} -- For storing keybind connections to disconnect when Anscript is destroyed

local SelectedTheme = AnscriptLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = AnscriptLibrary.Theme[Theme]
	...
	Anscript.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Anscript.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Anscript.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Anscript:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Anscript | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Anscript | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Anscript had an issue decoding the configuration file, please try delete the file and reopen Anscript.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(AnscriptLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Anscript | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--AnscriptLibrary:Notify({Title = "Anscript Flags", Content = "Anscript was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(AnscriptLibrary.Flags) do
...
function AnscriptLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Anscript | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			AnscriptLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show'.", Duration = 7, Image = 4400697855})
		else
			AnscriptLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping " .. tostring(getSetting("General", "AnscriptOpen")) .. ".", Duration = 7, Image = 4400697855})
		end
	end
...
local function saveSettings() -- Save settings to config file
	...
	if success then
		...
		callSafely(writefile, AnscriptFolder..'/settings'..ConfigurationExtension, encoded)
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Anscript Settings', 0, true)

	if TabList['Anscript Settings'] then
		TabList['Anscript Settings'].LayoutOrder = 1000
	end

	if Elements['Anscript Settings'] then
		Elements['Anscript Settings'].LayoutOrder = 1000
	end
...
function AnscriptLibrary:CreateWindow(Settings)
	if Anscript:FindFirstChild('Loading') then
		if getgenv and not getgenv().AnscriptCached then
			Anscript.Enabled = true
			Anscript.Loading.Visible = true

			task.wait(1.4)
			Anscript.Loading.Visible = false
		end
	end

	if getgenv then getgenv().AnscriptCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				AnscriptLibrary:Notify({Title = 'Build Mismatch', Content = 'Anscript may encounter issues as you are running an incompatible interface version ('.. ((Anscript:FindFirstChild('Build') and Anscript.Build.Value) or 'No Build') ..').\n\nThis version of Anscript is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Anscript"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Anscript Interface Suite" then
	...
	if not Settings.DisableAnscriptPrompts then
		task.spawn(function()
			while not AnscriptDestroyed do
				task.wait(math.random(180, 600))
				if AnscriptDestroyed then break end
				AnscriptLibrary:Notify({
					Title = "Anscript Interface",
	...
	if Settings.Discord and Settings.Discord.Enabled and not useStudio and not secureMode then
		ensureFolder(AnscriptFolder.."/Discord Invites")

		if not callSafely(isfile, AnscriptFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if requestFunc then
				pcall(function()
					requestFunc({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				callSafely(writefile, AnscriptFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Anscript RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		ensureFolder(AnscriptFolder.."/Key System")
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					...
					warn('Check docs.sirius.menu for help with Anscript specific development.')
				end
			end
		end
		...
		if callSafely(isfile, AnscriptFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				local savedKeys = callSafely(readfile, AnscriptFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension)
		...
		if not Passthrough and secureMode then
			warn("Anscript | Secure Mode: Key system requires a valid saved key. The key UI cannot be shown as it requires loading detectable assets.")
			Anscript.Enabled = false
			return AnscriptLibrary
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then
					...
					if Settings.KeySettings.SaveKey then
						callSafely(writefile, AnscriptFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						AnscriptLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	if Settings.KeySystem then
		...
		if AnscriptDestroyed then return end
	end
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				-- Prevents animation from trying to play if the button's callback called AnscriptLibrary:Destroy()
				if AnscriptDestroyed then
					return
				end
				if not Success then
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= AnscriptLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					AnscriptLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Anscript.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					AnscriptLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					AnscriptLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					AnscriptLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= AnscriptLibrary.Theme.Default then
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						AnscriptLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					AnscriptLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			AnscriptLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			AnscriptLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Anscript had an issue creating settings.') end
...
function AnscriptLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function AnscriptLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function AnscriptLibrary:Destroy()
	AnscriptDestroyed = true
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Anscript Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[getSetting("General", "AnscriptOpen")]) and not processed then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function AnscriptLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				AnscriptLibrary:Notify({Title = "Anscript Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			AnscriptLibrary:Notify({Title = "Anscript Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Anscript Configurations Error | '..tostring(result))
			AnscriptLibrary:Notify({Title = "Anscript Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	--local Window = AnscriptLibrary:CreateWindow({
	--	Name = "Anscript Example Window",
	--	LoadingTitle = "Anscript Interface Suite",
	--	Theme = 'Default',
	--	Icon = 0,
	...
	--		FileName = "Key", -- It is recommended to use something unique as other scripts using Anscript may overwrite your key file
	--		SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
	--		GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Anscript to get the key from
	...
	----AnscriptLibrary:Notify({Title = "Anscript Interface", Content = "Welcome to Anscript. These - are the brand new notification design for Anscript, with custom sizing and Anscript calculated wait times.", Image = 4483362458})
	...
	--local thoptions = {}
	--for themename, theme in pairs(AnscriptLibrary.Theme) do
...
task.delay(4, function()
	AnscriptLibrary.LoadConfiguration()
